pub const VIDEO_ENHANCE_SCRIPT: &str = r#"
(function() {
    'use strict';
    ...
    const fragmentShaderSource = `
        ...
        void main() {
            ...
            if (u_useCAS == 1) {
                // AMD FidelityFX Contrast Adaptive Sharpening (simplified)
    ...
    function showNotification(message) {
        let notification = document.getElementById('plex-enhance-notification');

        if (!notification) {
            notification = document.createElement('div');
            notification.id = 'plex-enhance-notification';
    ...
    function init() {
        ...
        try {
            const savedIndex = localStorage.getItem('plex-video-filter-index');
            if (savedIndex) {
                window.__videoEnhance.filterIndex = parseInt(savedIndex, 10) || 0;
            }

            const savedEnabled = localStorage.getItem('plex-video-enhance-enabled');
